(function(){
	...
	function init(){
		...
		enemy_dog_left.addAnimation("move", "0-10", true, 20);
		...
		enemy_hang_turn_left.addAnimation("move", "0-9", true, 8);
		...
		enemy_fly_right.addAnimation("move", "0-8", true, 24);
		...
		explosion.addAnimation("bomb", "0-14", true, 20);
		stage.addChild(explosion);
		explosion.x = 300;
		explosion.play("bomb");

		var ani = ["0-4,5-9", "10-25", "25-31,32-42,32-42,32-42,32-42"];
		var fps = [20, 30, 20, 20];
		for(var i = 0;i < ani.length;i ++)
		{
			player = new MovieClip;
			player.setImg(R.images.player, 58, 65);
			player.addAnimation("bomb", ani[i], true, fps[i]);
